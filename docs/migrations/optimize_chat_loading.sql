-- Migration: Optimize chat loading by adding first_photo_path to RPC function
-- This eliminates N+1 queries for item photos

-- Drop and recreate the function with first_photo_path included
...
BEGIN
  ...
  -- ✨ First photo path (NEW - eliminates N+1 query)
  LEFT JOIN LATERAL (
    SELECT path
    FROM item_photos
  ...
  ORDER BY COALESCE(m.created_at, c.created_at) DESC;  -- Order by last message time
END;
$$;
...
-- Add comment
COMMENT ON FUNCTION get_user_chats_with_details(uuid) IS 
'Get user chats with all details including first photo path. Optimized to eliminate N+1 queries.';
